...
$('#user_forget_getCode').click(function(){
	sendVcode();
});

function userLogin(){
	var user_forget_tel = $('#user_forget_tel').val();
	var user_forget_code = $('#user_forget_code').val();
	var user_forget_password = $('#user_forget_password').val();
	var user_forget_password_check = $('#user_forget_password_check').val();
	
	
	if(null == user_forget_tel || "" == user_forget_tel){
		$('#user_forget_error').html("<font color='red'>" +"请输入手机号！"+ "</font>");
		return;
	}
	
	if(null == user_forget_code || "" == user_forget_code){
		$('#user_forget_error').html("<font color='red'>" +"请输入验证码！"+ "</font>");
		return;
	}
	
	if(null == user_forget_password || "" == user_forget_password){
		$('#user_forget_error').html("<font color='red'>" +"请输入新的密码！"+ "</font>");
		return;
	}
	
	if(null == user_forget_password_check || "" == user_forget_password_check){
		$('#user_forget_error').html("<font color='red'>" +"请再次输入密码！"+ "</font>");
		return;
	}
	
	if(!phonehaoReg.test(user_forget_tel)){
		$('#user_forget_error').html("<font color='red'>" +"手机号格式错误！"+ "</font>");
		return;
	}
	
	if(user_forget_password != user_forget_password_check){
		$('#user_forget_error').html("<font color='red'>" +"两次密码输入不一致，请检查！"+ "</font>");
		return;
	}
	
	$.ajax({
     	type :"post" ,
     	url :"/qiji/users/forget",
     	data :{
     		utel : user_forget_tel,
     		vcode: user_forget_code,
     		upwd : user_forget_password
     	},
     	dataType: "json" ,
     	async :true ,
     	success: function(data) {
     		if(data.operSucc && data.succ){
     			...
     			myStorage.setItem("upwd",user_forget_password);
     			...
     			if(data.code == '001'){
     				alert("用户未注册，请先注册！");
     				window.location.href="/qiji/register";
     			}else{
...
function time(obj) {
	if(wait==0) {
		//$("#user_forget_getCode").removeAttr("disabled");
		$("#user_forget_getCode").attr("onclick","sendVcode(this)")
		$("#user_forget_getCode").html("获取验证码");
		wait = 60;
	}else {
		$("#user_forget_getCode").removeAttr("onclick");
		$("#user_forget_getCode").html(wait+"秒后重试");
		wait--;
	...
	$.ajax({
     	...
     	success: function(data) {
     		if(data.operSucc && data.succ){
     			wait = 60;
     			time(e);//倒计时
     			$('#user_forget_error').html("<font color='red'>" +""+ "</font>");
     		}else{
     			$('#user_forget_error').html("<font color='red'>" +data.message+ "</font>");
     		}
     	},
        error: function(data) {
        	$('#user_forget_error').html("<font color='red'>" +"系统错误！"+ "</font>");
       }
	});
